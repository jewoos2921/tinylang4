...
void Lexer::next(Token &Result) {
    ...
    if (charinfo::isIdentifierHead(*CurPtr)) {
        ...
        switch (*CurPtr) {
#define CASE(ch, tok) case ch: formToken(Result, CurPtr + 1, tok); break
            CASE('=', tok::equal);
            CASE('#', tok::hash);
            CASE('+', tok::plus);
            CASE('-', tok::minus);
...
#undef CASE
            case '(':
                if (*(CurPtr + 1) == '*') {
                    comment();
                    next(Result);
                } else formToken(Result, CurPtr + 1, tok::l_paren);
                break;
            case ':':
                if (*(CurPtr + 1) == '=') {
                    formToken(Result, CurPtr + 2, tok::colonequal);
                } else formToken(Result, CurPtr + 1, tok::colon);
                break;
            case '<':
                if (*(CurPtr + 1) == '=')
                    formToken(Result, CurPtr + 2, tok::lessequal);
                else formToken(Result, CurPtr + 1, tok::less);
                break;
            case '>':
                if (*(CurPtr + 1) == '=')
                    formToken(Result, CurPtr + 2, tok::greaterequal);
                else formToken(Result, CurPtr + 1, tok::greater);
                break;
            default:
